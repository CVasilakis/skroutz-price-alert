...
def _run(world: ShellWorld, *args: str):
    ...
    try:
        ...
        return subprocess.run(
            ["/bin/sh", str(checkout / "scripts/stop.sh"), *args],
            cwd=checkout,
            env=env,
            text=True,
            capture_output=True,
...
def test_help_documents_debug_and_preserves_outer_blank_lines():
    ...
    assert "Usage: stop.sh [-h] [--debug] [--<target> ...]" in result.stdout
...
def test_help_keeps_precedence_with_debug_in_every_position(args):
    ...
    assert "[+] Active executions" not in result.stdout
    _assert_standalone_frame(result.stdout)
...
def test_invalid_arguments_keep_exit_one_and_use_framed_status_output(args):
    ...
    assert_task_status(result.stdout, "i", "Run ./scrooge-alert stop --help for usage.")
    _assert_standalone_frame(result.stdout)


def test_duplicate_target_keeps_success_semantics_and_runs_once():
    result = _run(ACTIVE, "--skroutz", "--skroutz")

    assert result.returncode == 0, result.stdout + result.stderr
    assert result.stdout.count("Active execution stopped.") == 1


def test_no_active_execution_is_an_informational_noop():
...
def test_missing_systemctl_is_a_framed_preflight_failure():
    result = _run(replace(INSTALLED, tools="no-systemctl"))
...
def test_partial_failure_continues_to_other_targets_and_returns_one():
    world = ShellWorld(
        ...
        active_services=("skroutz", "amazon"),
        systemctl_fail=("stop",),
