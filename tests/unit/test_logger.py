...
import core.logger
from core.logger import NonEmptyFilter, get_target_logger, save_traceback
...
class TestGetTargetLogger(_LoggerCase):
    ...
    def test_quiet_log_lines_are_utc_labelled(self):
        ...
        logger.info("hello")
        content = (Path(core.logger.LOGS_DIR) / target / "output.log").read_text()
...
class TestSaveTraceback(_LoggerCase):
    ...
    def test_without_target_falls_back_to_the_root_error_log(self):
        ...
        content = (Path(core.logger.LOGS_DIR) / "errors.txt").read_text()
        self.assertIn("RuntimeError: boom", content)
