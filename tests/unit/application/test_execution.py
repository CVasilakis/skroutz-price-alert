...
from core.application.contracts import PriceOutcome, RunOutcome, RunReporter
from core.application.execution import ItemExecutor, policy_for
...
def test_unexpected_fault_exhausts_retries_and_affects_exit_status(monkeypatch):
    ...
    traceback = mock.Mock()
    monkeypatch.setattr("core.application.execution.save_traceback", traceback)
...
def test_rate_limit_exhaustion_aborts_target_and_sets_rate_status(monkeypatch):
    ...
    monkeypatch.setattr("core.application.execution.save_traceback", mock.Mock())
