...
try:
    from core.scrapers.support.http import HttpScraperClient
...
class TestRetryPreparation(unittest.TestCase):
    def test_diagnostics_exclude_request_and_identity_details(self):
        with (
            mock.patch("core.scrapers.support.http.tls_client.Session"),
            mock.patch(
                "core.scrapers.support.http.random.choice",
                return_value={
    ...
    def test_rotates_headers_and_replaces_session(self):
        ...
        with (
            mock.patch("core.scrapers.support.http.tls_client.Session", side_effect=[s1, s2]),
            mock.patch(
                "core.scrapers.support.http.random.choice", return_value={"User-Agent": "probe"}
            ),
        ):
            client = _ConcreteClient(_settings())
