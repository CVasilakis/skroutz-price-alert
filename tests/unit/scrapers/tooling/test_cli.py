...
def catalog():
    return synthetic_catalog(
        fake_plugin(name="insomnia", display_name="Insomnia"),
        fake_plugin(name="skroutz", display_name="Skroutz"),
    )


def test_schedule_missing_and_valid_config(tmp_path, catalog):
...
def test_catalog_and_schedules_are_independent(capsys, tmp_path, catalog):
...
def test_verifier_and_tooling_cli(capsys, tmp_path, catalog):
    assert cli_main(["catalog"], catalog=catalog) == 0
    output = capsys.readouterr().out
    assert "skroutz\tSkroutz\t" in output
    assert cli_main(["schedules", "--config-dir", str(tmp_path)], catalog=catalog) == 0
    assert "skroutz\thourly\tnocfg\t" in capsys.readouterr().out
    assert cli_main(["intervals"], catalog=catalog) == 0
