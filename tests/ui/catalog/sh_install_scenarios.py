...
_case("help", "Usage text with one flag row per registered target.",
      "--help", world=replace(ShellWorld(), plugins=("skroutz", "amazon")), tags=("help",))

_case("invalid_argument", "A positional argument is rejected before anything runs.",
      "foo", tags=("error",))

_case("python3_missing", "Prerequisite check: no python3 on PATH.",
      world=ShellWorld(tools="no-python3"), tags=("error",))

_case("venv_module_missing", "Prerequisite check: python3 exists but the venv module is unavailable.",
      world=ShellWorld(ensurepip_missing=True), tags=("error",))

_case("systemctl_missing", "Prerequisite check: systemd is not available.",
      world=ShellWorld(tools="no-systemctl"), tags=("error",))

_case("venv_create_fails", "python3 -m venv fails while creating a fresh environment.",
      world=ShellWorld(venv=False, venv_create_fails=True), tags=("error",))

_case("pip_upgrade_fails", "The pip self-upgrade inside the existing venv fails.",
      world=ShellWorld(pip_fail="upgrade"), tags=("error",))

_case("requirements_missing", "The root requirements.txt is gone.",
      world=ShellWorld(requirements_txt=False), tags=("error",))

_case("requirements_install_fails", "Installing the root requirements.txt fails.",
      world=ShellWorld(pip_fail="requirements"), tags=("error",))

_case("discovery_failed", "The venv is fine but plugin discovery raises - the diagnose branch.",
      world=ShellWorld(plugins=(), discovery_error=DISCOVERY_ERROR), tags=("error", "registry"))
...
_case("plugin_requirements_fail", "A plugin's own requirements.txt fails to install.",
      world=replace(_CONFIGURED,
                    requirements={"skroutz": "/opt/fake/scrapers/skroutz/requirements.txt"},
                    pip_fail="plugin"),
      tags=("error",))
...
_case("fresh_install_config_notes", "First-ever run: venv created, lingering enabled, config notes.",
