...
_case("stop_fails", "systemctl rejects a stop request; success is not reported.",
      world=replace(WORLD_INSTALLED, activating_services=("skroutz",),
                    systemctl_fail=("stop",)), tags=("error",))

_case("stop_noop_detected", "A successful stop that leaves the service running fails verification.",
      world=replace(WORLD_INSTALLED, activating_services=("skroutz",),
                    systemctl_noop=("stop",)), tags=("error",))

_case("query_fails", "A failed service-state query is not mistaken for inactivity.",
      world=replace(WORLD_INSTALLED, activating_services=("skroutz",),
                    systemctl_fail=("show",)), tags=("error",))

_case("batch_continues_after_failure", "A later independent target is still checked after failure.",
      world=ShellWorld(
          plugins=("skroutz", "amazon"), installed_services=("skroutz", "amazon"),
          activating_services=("skroutz",), systemctl_fail=("stop",),
      ), tags=("error",))
