...
from ui.harness.drivers import drive_not_installed, drive_orphan, drive_service
...
@scenario(
    Surface.STATUS,
    "service_healthy",
    "Installed, all settings valid, timer active, last run OK",
    tags=("ok",),
)
def _():
    return drive_service(
        TARGET, timer_props(True, NEXT_AT), _svc(), resolved_settings(), CFG, "hourly", "hourly"
    )
...
@scenario(
    Surface.STATUS,
    "service_defaults",
    "All settings unset (active defaults shown)",
    tags=("settings",),
)
def _():
...
@scenario(
    Surface.STATUS,
    "service_nocfg",
    "Config file missing (settings fall back to defaults)",
    tags=("settings",),
)
def _():
    resolved = resolved_settings(
...
@scenario(
    Surface.STATUS,
    "service_invalid_interval",
    "An unsupported execution_interval",
    tags=("settings",),
)
def _():
...
@scenario(
    Surface.STATUS,
    "service_invalid_retention",
    "An out-of-range log_retention_days",
    tags=("settings",),
)
def _():
...
@scenario(
    Surface.STATUS,
    "service_invalid_notify",
    "An unrecognized notify_scraping_errors value",
    tags=("settings",),
)
def _():
...
def _():
    return drive_service(
        TARGET, timer_props(False, NEXT_AT), _svc(), resolved_settings(), CFG, "hourly", "hourly"
    )
...
@scenario(
    Surface.STATUS, "exec_rate_limit", "Last run was rate limited (exit 17)", tags=("last_run",)
)
def _():
    return drive_service(
        TARGET,
...
@scenario(
    Surface.STATUS,
    "exec_scrape_error",
    "Last run exhausted a parser or unexpected scraper fault (exit 18)",
    tags=("last_run",),
)
def _():
    return drive_service(
...
@scenario(
    Surface.STATUS,
    "exec_notification_error",
    "Last run missed at least one notification (exit 20)",
    tags=("last_run",),
)
def _():
    return drive_service(
...
@scenario(
    Surface.STATUS,
    "exec_dependency_error",
    "Last run lacked scraper dependencies (exit 21)",
    tags=("last_run",),
)
def _():
    return drive_service(
...
@scenario(
    Surface.STATUS, "exec_interrupt", "Last run was interrupted (exit 130)", tags=("last_run",)
)
def _():
    return drive_service(
        TARGET,
...
@scenario(
    Surface.STATUS,
    "exec_unknown_code",
    "Last run failed with an unrecognized exit code",
    tags=("last_run",),
)
def _():
    return drive_service(
...
@scenario(
    Surface.STATUS, "not_scheduled", "Installed but the timer has no next elapse", tags=("timer",)
)
def _():
    return drive_service(
        TARGET, timer_props(True, "0"), _svc(), resolved_settings(), CFG, "hourly", "hourly"
    )


@scenario(
    Surface.STATUS, "never_run", "Installed and scheduled but never executed yet", tags=("ok",)
)
def _():
    return drive_service(
        TARGET,
...
@scenario(
    Surface.STATUS,
    "schedule_drift",
    "Live timer differs from the configured interval",
    tags=("timer",),
)
def _():
    # interval ok/default, but the on-disk OnCalendar ("*:0/30") != the configured one.
    return drive_service(
        TARGET, timer_props(True, NEXT_AT), _svc(), resolved_settings(), CFG, "hourly", "*:0/30"
    )


@scenario(
    Surface.STATUS,
    "drift_suppressed_invalid_interval",
    "An invalid interval suppresses the drift footnote",
    tags=("timer", "settings"),
)
def _():
    ...
    resolved = resolved_settings(interval=("1h", STATUS_INVALID, "3h"))
    return drive_service(
        TARGET, timer_props(True, NEXT_AT), _svc(), resolved, CFG, "hourly", "*:0/30"
    )
...
@scenario(
    Surface.STATUS,
    "service_many_issues",
    "Invalid setting + timer down + failed run + drift",
    tags=("combined",),
)
def _():
    resolved = resolved_settings(
...
@scenario(
    Surface.STATUS, "not_installed", "Plugin registered but no units installed", tags=("error",)
)
def _():
...
@scenario(
    Surface.STATUS, "orphan", "Units installed for a plugin no longer registered", tags=("orphan",)
)
def _():
