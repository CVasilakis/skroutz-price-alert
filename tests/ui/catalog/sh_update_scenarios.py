...
_case(
    "clean_happy_path",
    "Clean tree already on 'main': the plainest, most common update.",
    world=_BASE,
    tags=("ok",),
)
...
_case(
    "invalid_argument",
    "Any argument other than -h/--help is rejected.",
    "foo",
    world=_BASE,
    tags=("error",),
)
...
_case(
    "no_systemctl",
    "A systemd-less host is refused before the destructive git reset.",
    world=replace(_BASE, tools="no-systemctl"),
    tags=("error",),
)

_case(
    "stop_fails_before_git",
    "A scraper that cannot stop aborts before checkout/reset.",
...
_case(
    "dirty_declined",
    "Uncommitted changes detected; the user answers no.",
...
_case(
    "dirty_confirmed",
    "Uncommitted changes discarded on confirmation; full update runs.",
    world=replace(_BASE, git_dirty=True),
    stdin="y\n",
)

_case(
    "branch_switch_notice",
    "A clean tree on another branch is switched to 'main'.",
...
_case(
    ...
    world=replace(_BASE, git_fail=("checkout",)),
    tags=("error",),
)

_case("fetch_fails", "git fetch fails.", world=replace(_BASE, git_fail=("fetch",)), tags=("error",))
...
_case(
    ...
    "The nested install.sh run fails mid-update.",
...
_case(
    ...
    "The new version ships a scraper the user has not installed.",
