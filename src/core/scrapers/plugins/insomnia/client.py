...
from core.scrapers.support.pricing import parse_price
...
class Client(HttpScraperClient):
    ...
    def _parse_adverts(
        self, html: str, listing_url: str, include: tuple[str, ...], exclude: tuple[str, ...]
    ) -> list[Offer]:
        ...
        if not adverts:
            raise ScraperParseError(
                "No adverts found on the listing page (markup change or blocked?)"
            )
        ...
        for advert in adverts:
            ...
            advert_url = urljoin(listing_url, href.strip())
