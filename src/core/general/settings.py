...
appended to :data:`GENERAL_SETTING_SPECS` - no new resolver, model or config-check code.

Import-light: stdlib only, like the settings package it builds on, so it is safe to
import from ``--status`` and the config panel.
"""

import os
import re

from core.scrapers.base.settings import ResolvedSettings, SettingSpec, resolve_all
...
# Canonical reminder cadences, each mapped to its whole-week count (None = disabled).
# Whole weeks keep ``slot + interval`` on the Saturday-13:00 slot grid by construction
# (see core.general.reminder), so "1 month" means "every 4th Saturday" - no month-length
# or leap-year arithmetic anywhere.
SUPPORTED_REMINDERS: dict[str, int | None] = {
    "off": None,
    "1w": 1,
    "1m": 4,
    "3m": 13,
...
_REMINDER_DISPLAY: dict[str, str] = {
    "off": "off",
    "1w": "1 week",
    "1m": "1 month",
    "3m": "3 months",
...
def reminder_warning_message() -> str:
    """The single user-facing message for an unsupported ``reminder`` value."""
    return f"Unsupported reminder value. Using the default ({_REMINDER_DISPLAY[DEFAULT_REMINDER]})."


def reminder_day_warning_message() -> str:
    """The single user-facing message for an unsupported ``reminder_day`` value."""
    return f"Unsupported reminder_day value. Using the default ({DEFAULT_REMINDER_DAY})."


def reminder_time_warning_message() -> str:
    """The single user-facing message for an unsupported ``reminder_time`` value."""
    return f"Unsupported reminder_time value. Using the default ({DEFAULT_REMINDER_TIME})."
...
SPEC_REMINDER = SettingSpec(
    ...
    warning=reminder_warning_message(),
...
SPEC_REMINDER_DAY = SettingSpec(
    ...
    warning=reminder_day_warning_message(),
...
SPEC_REMINDER_TIME = SettingSpec(
    ...
    warning=reminder_time_warning_message(),
